...
def read_csv(filename):
    ...
    with open(filename, 'r') as f:
        ...
        for row in reader:
            ...
            # Optionally skip smaller sizes if needed, here we keep all
...
# Load data from ArrayList CSV
arraylist_data = read_csv('list_performance.csv')

methods = sorted(arraylist_data.keys())
sizes = sorted([size for size, _ in arraylist_data[methods[0]]])
...
# Set x-axis ticks every 10,000 from 10,000 to max size (100,000 or more if data)
x_ticks = list(range(10000, max(sizes)+1, 10000))

for idx, method in enumerate(methods):
    times = [time for _, time in arraylist_data[method]]
    ax = axes[idx]

    ax.plot(sizes, times, label='ArrayList', linestyle='-', linewidth=1.5)
    ...
    # Set x-axis ticks, formatted with commas, rotated for readability
...
plt.tight_layout()
plt.savefig('ArrayList_Performance_Comparisons.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"All performance charts saved in 'ArrayList_Performance_Comparisons.png' for methods: {', '.join(methods)}")
